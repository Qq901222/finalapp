...
// 幣別清單（可自行增減）
const CURRENCIES = ['TWD','USD','EUR','JPY','HKD','CNY','GBP','AUD','CAD','SGD','KRW'];

// 幣別小工具：一排 chip
function CurrencyChipsInline({ value, onChange }) {
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 6, marginBottom: 2 }}>
          ...
          return (
            <TouchableOpacity
              ...
              style={{
                ...
                borderColor: active ? '#FFC107' : '#ddd',
                backgroundColor: active ? '#FFE082' : '#fff',
                marginRight:8
              }}
              activeOpacity={0.85}
            >
              <Text style={{ fontSize:13, fontWeight: active ? '800' : '600', color: '#333' }}>{code}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
const takePhotoAndFill = async () => {
  try {
    ...
    console.warn('takePhotoAndFill error', e?.message || e);
    Alert.alert('拍照失敗', '請稍後再試');
  }
};

const openReceiptChooser = () => {
  Alert.alert('收據記帳', '選擇方式', [
    { text: '拍照', onPress: takePhotoAndFill },
    { text: '選照片', onPress: pickImageAndFill },
