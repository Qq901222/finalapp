...
export default function GroupDetailScreen({ navigation, route }) {
  ...
  // --- 外幣換算器 ---
  const [fxOpen, setFxOpen] = useState(false)     // 是否展開換算器
  const [fxFrom, setFxFrom] = useState('USD')     // 選擇來源幣別
  const [fxInput, setFxInput] = useState('')      // 使用者輸入的外幣金額
  const [fxPreview, setFxPreview] = useState(null)// 預覽換算結果
  const [fxLoading, setFxLoading] = useState(false) // loading 狀態

  async function applyFx() {
    const val = Number(fxInput || 0)
    if (!val || isNaN(val)) return
    try {
      setFxLoading(true)
      const r = await convertCurrency(val, fxFrom, 'TWD')
      if (r?.result) {
        setExpenseTotal(String(r.result)) // 寫入「總金額」
        setFxPreview(r.result)            // 顯示換算提示
      }
    } catch (e) {
      console.warn('fx convert error', e?.response?.data || e.message)
      Alert.alert('換算失敗', '請稍後再試')
    } finally {
      setFxLoading(false)
    }
  }
    ...
    <KeyboardAvoidingView style={styles.full} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {/* --- 外幣換算器區塊 --- */}
          <TouchableOpacity onPress={() => setFxOpen(v => !v)} style={{ marginBottom: 8 }}>
            <Text style={{ color: '#0288D1', fontWeight: '700' }}>
              {fxOpen ? '隱藏外幣換算器' : '用外幣換算填入金額'}
            </Text>
          </TouchableOpacity>

          {fxOpen && (
            <View style={{
              marginVertical: 12, padding: 12, borderWidth: 1,
              borderRadius: 8, borderColor: '#ddd', backgroundColor:'#FAFAFA'
            }}>
              <Text style={{ fontWeight:'700', marginBottom:6 }}>來源幣別</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                {['USD','EUR','JPY','HKD','CNY','TWD'].map(code => {
                  const active = fxFrom === code
                  return (
                    <TouchableOpacity
                      key={code}
                      onPress={() => setFxFrom(code)}
                      style={{
                        paddingHorizontal:12, paddingVertical:6, borderRadius:999, borderWidth:1,
                        borderColor: active ? '#4CAF50' : '#ddd',
                        backgroundColor: active ? '#A5D6A7' : '#fff',
                        marginRight:8
                      }}
                    >
                      <Text style={{ fontWeight: active ? '800' : '600' }}>{code}</Text>
                    </TouchableOpacity>
                  )
                })}
              </ScrollView>
              ...
              <TouchableOpacity
                style={{ backgroundColor:'#FFECB3', padding:10, borderRadius:8, alignItems:'center' }}
                onPress={applyFx}
              >
                <Text style={{ fontWeight:'700', color:'#5d4a00' }}>套用換算值到金額 (TWD)</Text>
              </TouchableOpacity>

              <Text style={{ marginTop: 8, color:'#6b7280' }}>
                {fxLoading ? '換算中…' : (fxPreview ? `≈ TWD ${fxPreview}` : '')}
              </Text>
            </View>
          )}
                ...
                <View key={`rm-${group.to}`} style={{ marginBottom: 8 }}>
                  ...
                  {group.items.map((p, i) => (
                    <View key={`rm-${group.to}-${p.from}-${i}`} style={styles.debtRow}>
                      ...
                      <Text style={{ color: '#E65100', fontWeight: '800' }}>
                      NT${fmt0(group.total * (exchangeRate || 1))}
                      </Text>
                      <TouchableOpacity
                        style={styles.repayBtn}
